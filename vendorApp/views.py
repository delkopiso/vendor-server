...
def get_weddings_by_region(region, limit, offset):
    base_query = Article.objects(region=region, section='Weddings')
    query_size = len(base_query)
    return base_query.order_by('-dateAdded', 'mixIndex').skip(offset).limit(limit), query_size


def get_section_articles_combo(region, sectionA, sectionB, sectionC, sectionD, sectionE, sectionF, sectionG, sectionH, sectionI, sectionJ, sectionK, sectionL, sectionM, sectionN, limit, offset):
...
def generate_section_combo_output(query_func, region, sectionA, sectionB, sectionC, sectionD, sectionE, sectionF, sectionG, sectionH, sectionI, sectionJ, sectionK, sectionL, sectionM, sectionN, request):
    current_page = int(request.GET.get(PAGE_NUMBER_PARAM, FIRST_PAGE))
    current_page = current_page if current_page > FIRST_PAGE else FIRST_PAGE
    page_size = int(request.GET.get(PAGE_SIZE_PARAM, DEFAULT_PAGE_SIZE))
    offset = page_size * (current_page - 1)
    results = query_func(region, sectionA, sectionB, sectionC, sectionD, sectionE, sectionF, sectionG, sectionH, sectionI, sectionJ, sectionK, sectionL, sectionM, sectionN, page_size, offset)
    count = results[1]
    last_page = int(math.ceil(count / page_size))
    prev_page = current_page - 1 if current_page > FIRST_PAGE else None
    next_page = current_page + 1 if current_page < last_page else None
...
def get_region_startup(request, region):
    ...
    weddings = ArticleSerializer(get_weddings_by_region(region, page_size, offset)[0], many=True).data
    content = {
        "trending": trending,
        "gossip": gossip,
        "tech": tech,
...
def get_logo_all(request, region):
    ...
    weddings = get_region_logos_for_section_all(region, "Weddings")[0]
...
@api_view(['GET'])
@renderer_classes((JSONRenderer,))
def get_region_weddings(request, region):
    return Response(generate_output(get_weddings_by_region, region, request))  

@api_view(['GET'])
@renderer_classes((JSONRenderer,))
def get_region_section_combo(request, region, sectionA, sectionB, sectionC="", sectionD="", sectionE="", sectionF="",sectionG="",sectionH="",sectionI="",sectionJ="",sectionK="",sectionL="",sectionM="", sectionN=""):
    return Response(generate_section_combo_output(get_section_articles_combo, region, sectionA, sectionB, sectionC, sectionD, sectionE, sectionF, sectionG, sectionH, sectionI, sectionJ, sectionK, sectionL, sectionM, sectionN, request))
